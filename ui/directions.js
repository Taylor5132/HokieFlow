...
export function mountDirections(root,{selection={origin:'current',destination:'',mode:'walk'},destinationHint=''}={}){
 ...
 root.innerHTML=`<div class="route-fields"><div class="route-field"><span class="route-point" aria-hidden="true">A</span><div><label for="route-origin">From</label><select id="route-origin"></select></div><button type="button" class="find-me" aria-label="Find my location">${pin}<span>Find me</span></button></div><div class="route-field"><span class="route-point destination-point" aria-hidden="true">B</span><div><label for="route-destination">To</label><select id="route-destination"></select></div></div></div><p class="route-location helper" role="status"></p><div class="route-modes" role="group" aria-label="Travel mode"><button type="button" data-route-mode="walk">${walkIcon} Walk</button><button type="button" data-route-mode="bus">${busIcon} Bus</button></div><a class="primary campus-directions" target="_blank" rel="noopener noreferrer">Find campus route <span aria-hidden="true">↗</span></a><p class="route-provider">Opens in Apple Maps</p><p class="route-error helper" role="alert"></p>`;
