...
const state = {tab:'home',answer:null,clock:null,clockFailed:false,busy:false,error:'',query:'',review:'fits',status:null, savedClass:null, homeData:null, homeError:'',user:null,accountReady:false,accountError:'',accountStorageError:'',accountData:{savedClass:null,reduceMotion:false,plans:[],events:[]},accountVersion:0,accountSaving:false,classQuery:'',classResults:null,classBusy:false,classError:''};
...
async function getJSON(path,options={}){const response=await fetch(`${config.apiBase}${path}`,{...options,signal:AbortSignal.timeout(path.startsWith('/api/transit/')?18000:70000)});const body=await response.json();if(!response.ok && !body.clarification && body.feasible!==false)throw new Error(body.error||'The planner could not respond. Please try again.');return body;}
