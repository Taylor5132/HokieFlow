...
document.addEventListener('click',event=>{
 ...
 if(action==='refresh-events'){void loadCampusEvents();return;}
...
function applyAccount(result){state.user=result.user?{...result.user,name:result.user.name||result.user.user_metadata?.name||result.user.email?.split('@')[0]||'Hokie'}:null;state.accountData=result.data||{savedClass:null,reduceMotion:false,plans:[],events:[]};state.accountData.events??=[];state.accountVersion=result.version||0;state.savedClass=state.accountData.savedClass;document.body.classList.remove('reduce-motion');state.accountError='';}
...
async function loadBuses(){
 ...
 try{
  ...
  const fresh=valid.filter(r=>Number.isFinite(Date.parse(r.value.fetched_at))&&Date.now()-Date.parse(r.value.fetched_at)<=120000);if(!fresh.length)throw new Error();
  transitDepartures=fresh.flatMap(r=>r.value.departures||[]);transitUpdated=Math.min(...fresh.map(r=>Date.parse(r.value.fetched_at)));
...
function campusEventsPage(){
 ...
 return `${heading('','Events','')}<div class="section-title"><button class="text-button" data-tab="more">Back to More</button><button class="text-button" data-action="refresh-events" ${campusEventState==='loading'?'disabled':''}>Refresh</button></div><p class="helper">Virginia Tech · Times in Eastern time</p>${campusEventState==='loading'?'<p role="status" class="helper">Loading events…</p>':''}${campusEventError?`<p class="error" role="alert">${escape(campusEventError)}</p>`:''}${campusEventNotices.map(n=>`<p class="helper">${escape(n)}</p>`).join('')}<div class="campus-events">${campusEvents.map(e=>{
