import {loadPreferences,savePreferences,dueReminders} from './preferences.js';
...
function greeting(){
 const iso=state.clock?.iso||state.clock?.evaluated_at;
 if(!iso||!Number.isFinite(Date.parse(iso))){const h=Number(new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',hour:'numeric',hourCycle:'h23'}).format(new Date()));return h<12?'Good morning,':h<17?'Good afternoon,':'Good evening,';}
...
function moreContent(){return `${heading('','Settings','')}<section class="settings" aria-label="App settings"><div class="appearance-setting"><div><strong>Appearance</strong><small>Pick your favorite look.</small></div><div class="theme-options" role="group" aria-label="Color theme"><button data-theme="light" aria-pressed="${preferences.theme==='light'}">Light</button><button data-theme="dark" aria-pressed="${preferences.theme==='dark'}">Dark</button></div></div><button data-action="notifications" role="switch" aria-checked="${preferences.notifications&&'Notification' in window&&Notification.permission==='granted'}" ${notificationBusy?'disabled':''}><span><strong>Notifications</strong><small>Class and event reminders while the app is open.</small></span><span class="switch" aria-hidden="true"></span></button>${notificationMessage?`<p class="helper" role="status">${escape(notificationMessage)}</p>`:''}<button data-tab="about"><span><strong>About HokieFlow</strong><small>A little help getting around campus.</small></span>${icon('arrow')}</button></section><section class="settings-account"><h2 class="eyebrow">Account</h2>${accountPanel()}</section>`;}
...
function render(){
 const page=state.tab==='login'?`${heading('',state.authMode==='register'?'Create account':'Log in','')}<section class="login-page">${authBody(state.authMode||'login')}</section>`:state.tab==='about'?aboutContent():state.tab==='home'?`<header class="home-header"><div><p class="eyebrow">${escape(new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',weekday:'short',month:'short',day:'numeric'}).format(new Date()))}</p><h1>${greeting()}<br><span>Hokie<b>.</b></span></h1></div><div id="home-weather" class="home-weather">${weatherMarkup()}</div></header>${state.homeError?`<p class="notice">${escape(state.homeError)}</p>`:''}${upcomingCard()}<div id="home-transit">${nearbyBusCard()}</div>${homeDining()}${state.answer||state.busy?planResult():''}`:state.tab==='plan'?schedulePage():state.tab==='dining'?`${heading('','Dining','')}${diningDirectory()}`:state.tab==='bus'?`${heading('','Bus','')}${busCard()}${busContent()}<a class="text-button bt-service-link" href="https://ridebt.org/routes-schedules" target="_blank" rel="noreferrer">BT schedules & service ${icon('arrow')}</a>`:moreContent();
 directionsPanel?.destroy();directionsPanel=null;
 $('#main').innerHTML=`<div class="page-enter">${state.tab==='login'?'':accountBar()}${state.error?`<p role="alert" class="error">${escape(state.error)}</p>`:''}${page}${sourceNote()}${state.tab==='home'?'<p class="copyright">© HokieFlow all rights reserved 2026</p>':''}</div>`;
...
async function getJSON(path,options={}){const response=await fetch(`${config.apiBase}${path}`,{...options,signal:AbortSignal.timeout(70000)});const body=await response.json();if(!response.ok && !body.clarification && body.feasible!==false)throw new Error('The planner could not respond. Please try again.');return body;}
...
document.addEventListener('click',event=>{
 ...
 if(action==='retry-dining')void loadDining();
...
if(preview){state.clock=previewTime;render();}else{render();void Promise.allSettled([syncClock(),getJSON('/api/status').then(status=>{state.status=status;})]).then(()=>render());setInterval(syncClock,30000);setInterval(()=>{const c=state.clock;if(!c||c.is_replay||c.pinned||!c.ticking||state.clockFailed)return;const server=Date.parse(c.iso||c.evaluated_at);if(!Number.isFinite(server))return;const label=$('#clock-label');if(label)label.textContent=`Campus ${new Intl.DateTimeFormat('en-US',{timeZone:c.timezone||'America/New_York',hour:'numeric',minute:'2-digit',second:'2-digit'}).format(new Date(server+performance.now()-syncMoment))} · server synced`;},1000);}
...
function applyAccount(result){state.user=result.user;state.accountData=result.data||{savedClass:null,reduceMotion:false,plans:[],events:[]};state.accountData.events??=[];state.accountVersion=result.version||0;state.savedClass=state.accountData.savedClass;document.body.classList.remove('reduce-motion');state.accountError='';}
...
function authBody(mode){return `<p>${mode==='register'?'Create an account to save your classroom and plans.':'Log in to pick up your saved class and plans.'}</p><button class="google-signin" type="button" data-action="google-login"><span class="google-mark" aria-hidden="true">G</span>Continue with Google</button><div class="auth-divider"><span>or use email</span></div><p id="google-auth-error" class="auth-message" role="alert"></p><form id="auth-form" class="class-form" data-mode="${mode}">${mode==='register'?'<label>Your name<input name="name" autocomplete="name" maxlength="60" required></label>':''}<label>Email<input name="email" type="email" autocomplete="email" maxlength="254" required></label><label>Password<input name="password" type="password" autocomplete="${mode==='register'?'new-password':'current-password'}" minlength="12" maxlength="128" required></label><p class="helper">${mode==='register'?'Use 12–128 characters.':'Use your HokieFlow password.'}</p><p id="auth-error" role="alert"></p><button class="primary" type="submit">${mode==='register'?'Create account':'Log in'} ${icon('arrow')}</button></form><button class="text-button" data-action="${mode==='register'?'login':'signup'}">${mode==='register'?'Already have an account? Log in':'New here? Create an account'}</button>`;}
async function submitAuth(form){const button=form.querySelector('[type="submit"]');button.disabled=true;const fields=new FormData(form);const payload=Object.fromEntries(fields);$('#auth-error').textContent='';try{const result=await accountRequest(`/api/auth/${form.dataset.mode}`,payload);applyAccount(result);state.answer=null;state.query='';if($('#detail').open)closeSheet();navigate('home');announce('Logged in. Your saved data is ready.');}catch(error){const target=$('#auth-error');if(target)target.textContent=error.message;}finally{button.disabled=false;}}
async function saveAccount(data){if(!state.user)throw new Error('Log in to save your data.');if(state.accountSaving)throw new Error('A save is already in progress.');state.accountSaving=true;try{applyAccount(await accountRequest('/api/account/data',{data,version:state.accountVersion},'PUT'));}finally{state.accountSaving=false;}}
...
function locateDining(){if(!navigator.geolocation){diningError='Location is unavailable. You can still browse campus dining.';render();return;}diningLoading=true;diningError='';render();navigator.geolocation.getCurrentPosition(p=>{diningLocation={lat:p.coords.latitude,lon:p.coords.longitude,accuracy:p.coords.accuracy};diningLoading=false;if(!diningLoaded)void loadDining();else render();},()=>{diningLoading=false;diningError='Location wasn’t available. Browse all campus dining or try again.';render();},{timeout:10000,maximumAge:60000,enableHighAccuracy:false});}
function diningDirectory(){const places=nearbyDining(diningPlaces,diningLocation);return `<div class="dining-location"><button class="primary" data-action="nearby-dining" ${diningLoading?'disabled':''}>${diningLoading?'Finding your options…':diningLocation?'Refresh my location':'Find dining near me'} ${icon('pin')}</button>${diningLocation?`<button class="text-button" data-action="all-dining">Show all campus dining</button><p class="helper">Location accuracy ±${Math.round(diningLocation.accuracy)} m · distances are straight-line estimates.</p>`:''}</div>${diningError?`<p class="notice" role="alert">${escape(diningError)}</p><button class="text-button" data-action="retry-dining">Try again</button>`:''}${!places.length&&!diningLoading&&!diningError?'<div class="empty"><h2>Your next favorite spot.</h2><p>No dining locations to show right now. Check back soon.</p></div>':''}<div class="dining-list">${places.map(p=>`<article class="dining-place"><div class="section-title"><h2>${escape(p.name)}</h2>${p.distance!==null?badge(distanceFeet(p.distance)+' away'):''}</div><p>${escape(p.building||p.address||'Campus dining')}</p>${p.hours_text?`<p class="helper">${escape(p.hours_text)}</p>`:''}${p.description?`<p>${escape(p.description)}</p>`:''}${p.updated_at?`<p class="helper">Updated ${escape(p.updated_at)}</p>`:''}<button class="text-button" data-dining-place="${escape(p.building||p.name)}">Get directions ${icon('arrow')}</button></article>`).join('')}</div>`;}
...
async function loadBuses(){
 if(transitBusy){busRefreshQueued=true;return;}
 const version=busLocationVersion,location=transitLocation||CAMPUS;
 transitBusy=true;transitError='';updateHomeData();
 try{
  if(!transitStops.length){const data=await getJSON('/api/transit/stops');transitStops=data.stops||[];}
  ...
  const valid=responses.filter(r=>r.status==='fulfilled');if(!valid.length)throw new Error();
  transitDepartures=valid.flatMap(r=>r.value.departures||[]);transitUpdated=Date.now();
  if(valid.length!==responses.length)transitError='Some stop times couldn’t refresh.';
 ...
 finally{transitBusy=false;updateHomeData();if(busRefreshQueued){busRefreshQueued=false;void loadBuses();}}
}
...
void loadDining();void loadWeather();void loadBuses();setInterval(()=>{if(!document.hidden){void loadBuses();}},30000);setInterval(()=>{if(!document.hidden)void loadWeather();},600000);setInterval(updateHomeData,10000);

async function toggleNotifications(){
 if(notificationBusy)return;
 if(preferences.notifications){preferences.notifications=false;notificationMessage='Reminders turned off.';}
 else if(!('Notification' in window)||!window.isSecureContext){notificationMessage='This browser does not support reminders. Try a supported browser on HTTPS.';}
 else{notificationBusy=true;try{const permission=await Notification.requestPermission();preferences.notifications=permission==='granted';notificationMessage=preferences.notifications?'You’ll get a reminder 10 minutes before a saved event while HokieFlow is open.':permission==='denied'?'Notifications are blocked. You can allow them in your browser’s site settings.':'Notifications stayed off.';}catch{preferences.notifications=false;notificationMessage='Notifications aren’t available in this browser.';}finally{notificationBusy=false;}}
 savePreferences(preferenceStorage,preferences);if(state.tab==='more'){render();$('[data-action="notifications"]')?.focus();}checkReminders();
}
function checkReminders(){
 if(!preferences.notifications||!state.user||!('Notification' in window)||Notification.permission!=='granted')return;
 for(const r of dueReminders(events())){const key=state.user.id+':'+r.key;if(sentReminders.has(key))continue;try{const n=new Notification(r.title,{body:`Starts in ${r.minutes} min.`,tag:key});sentReminders.add(key);n.onclick=()=>{window.focus();navigate('plan');n.close();};}catch{preferences.notifications=false;savePreferences(preferenceStorage,preferences);notificationMessage='This browser cannot show reminders. Try another supported browser.';if(state.tab==='more')render();break;}}
}
