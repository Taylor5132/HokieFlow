...
try:  # optional third-party stack; the rest of the suite is stdlib-offline
    from auth import (
        register_user,
        login_user,
        logout_user,
    ...
    AUTH_IMPORT_ERROR = _exc
    register_user = login_user = logout_user = require_auth = None
    start_google_oauth = handle_google_oauth_callback = None
    get_session_cookie = set_session_cookie = None
...
@unittest.skipIf(AUTH_IMPORT_ERROR is not None,
                 f"auth needs supabase + python-dotenv (pip install -r "
                 f"requirements.txt): {AUTH_IMPORT_ERROR}")
