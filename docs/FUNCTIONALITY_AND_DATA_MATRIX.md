...
| **E. Discover later** | Browse public classes / import CRNs or ICS | 🟢 Search shipped | Live `GET /api/classes/search` over the public Banner timetable (one rate-limited POST, cached), offline answers from committed captures, `search_classes` agent tool, Schedule-tab search box that adds a section to the schedule; ICS import and conflicts implemented |
...
| "How many calories / protein?" | `nutrition_for_location`, `nutrition_bulk` | ✅ live / 🟡 replay | `cals, protein_g, fat_g, carb_g, sodium_mg` | VT NutritiveReport API | On demand, cached | Real macros in LIVE mode; fixture total for `214022*1*1,141002*2*1` = **479.616 kcal**. In REPLAY the D2 nutrition fixtures were captured after the pinned clock, so they are refused as non-contemporaneous and `kcal` is `None` | Values arrive as strings; missing values sent as `----` → treated as 0; future-captured nutrition never populates replay rows/gold | kcal + protein with "VT menu API" badge; "—" when unknown |
| "Is it open? When does it close?" | `get_hours` / `dining.hours` / `is_open` / `dining.open_windows` | 🟡 (D2) | `foodpro_id, name, date, open_time, close_time, is_open_now, closes_in_min` | VT hours API | Daily | Hours ↔ menu join via `foodpro_id == locationNum` (verified D2 = 15). Each window is anchored to its own source date; an early-morning probe loads D-1's ACTUAL overnight windows, and a missing D-1 is UNKNOWN | Two windows only in the fixture: 09:30:01–15:00:00, 15:00:01–20:00:00; D's windows are never shifted backward | Open/closed, "closes in N min" |
...
| NWS points metadata | 24 h | `weather.POINTS_TTL_S` | Implemented backend |
| NWS hourly forecast | 25 min | `weather.HOURLY_TTL_S` | Implemented backend; no replay fixture |
| NWS alerts | 3 min | `weather.ALERTS_TTL_S` | Implemented backend; no replay fixture |
| NWS observation | 10 min | `weather.OBSERVATION_TTL_S` | KBCB is airport data, not on-campus |
...
| **Classes / calendar** | `hokieday.classes`: public Banner HTML parser, deterministic query parser, snapshot search, CRN selection, bounded ICS import, conflicts, schedule records and next-class contract | 🟢 | Live `GET /api/classes/search` (cache/ holds live captures; fixtures/ holds committed snapshots); public POST `HZSKVTSC.P_ProcRequest`; user-provided ICS/CRNs | Per term / user import / 10-minute live cache | Answers say `source: banner_live` or `snapshot` with the capture's `fetched_at`; only term 202609 has a verified window; Banner weekly recurrence is explicitly assumed and excluded from deadlines by default; no personal VT system access | Figma may use documented loading/TBA/conflict/stale/recurrence-unavailable states; never claim automatic personal schedule sync or live registration data |
...
| B2 | ✅ **DONE (basic) — multi-location dining layer** | Broadens the food picker and the closed/empty/unavailable/future-capture states | Shipped: 12-location directory; `location_status` with independent `menu_status`/`hours_status` and composite `ok`/`closed`/`empty`/`unavailable`; future-captured and source-mismatched payloads rejected; `list_foods`/`search_foods`/`filter_foods` basic rows with provenance; `find_food(location_num=None)` typed `sources_ok`/`sources_skipped`, and `max_kcal` across all locations refuses unproven items. **Replay is honestly D2-only** (all other halls unavailable); multi-location behavior is proven with a synthetic temp cache in tests, and live mode can fetch all 12. New-hall nutrition is deliberately NOT committed. Future-captured NUTRITION is also refused, so replay kcal is honestly `None` and a hard `max_kcal` returns typed `nutrition_unavailable` rather than unproven rows; overnight windows are anchored to their source date and an early probe loads D-1's real hours (missing D-1 = unknown) | Data capture + seed |
...
| B10 | 🟢 **Class search shipped** | Deterministic query parser, live Banner search with cache + rate limit, offline snapshot answers, `search_classes` agent tool, Schedule-tab search box that adds a section to the schedule | Remaining: schedule persistence per CRN, VT GIS classroom join, explicit Banner term-assumption consent | Public Banner + user-controlled ICS |
