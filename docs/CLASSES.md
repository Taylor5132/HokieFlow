...
| Final-exam schedule | `https://selfservice.banner.vt.edu/ssb/hzskexam.P_DispExamInfo` | GET | exam code → date/time |
...
| Fixture | Contents |
|---|---|
| `fixtures/classes_timetable_fall2026_as.html` | redacted `<table …>` fragment of a harmless `subj_code=AS` query, captured 2026-09-19. Every instructor cell was `N/A`; the public page has no student data. |
| `fixtures/classes_buildings.html` | the public building-abbreviation table (295 codes). |
| `fixtures/classes_snapshot_fall2026_as.json` | a generated snapshot envelope wrapping the fragment, `fetched_at=2026-09-19T15:22:29Z`. |
...
- `expand_section(section, *, start, end, tz, skip_holidays)` /
  `expand_sections(...)` — weekly expansion of `Meeting.days` across an
  inclusive date range. Skips verified non-instruction days (`HOLIDAYS`).
  TBA/ARR meetings expand to **nothing** (they have no day/time).
...
`none`. TBA meetings are surfaced via `ClassSection.flags().has_tba`, because a
TBA meeting has no time to schedule against.

**Schedule (no DB; JSON/Lakebase-ready records)**
- `add_to_schedule(schedule, sections, crns, *, term, snapshot_id)` → schedule
  list + `added` / `missing` / `invalid` / `already_in_schedule`.
- `remove_from_schedule(schedule, crns)` → `removed` / `not_in_schedule`.
- `resolve_schedule(schedule, sections)` — rejoin by (term, crn).
- `schedule_json(schedule, sections, *, start, end, snapshot, now)` → adds
  `conflicts`, `state`, `unresolved_crns`.
...
- `parse_ics(text, *, tz) -> IcsParse(events, warnings, errors, valid)`.
  ...
  `COUNT`/`UNTIL`. Anything else (e.g. `FREQ=MONTHLY`, `BYSETPOS`, missing
...
- `IcsEvent.to_section()` — imported blocks use the UID as identity and carry a
  `"not a VT CRN"` warning; `source="ics"`.
...
`DEMO_MODE=cache` can replay. `--max-subjects N` is a safety valve.
...
1. **Wire `next_class_json` into the planner.** In `hokieday/tools.py` add a
   ...
   `classes.next_class_json(...)["deadline"]`. Keep the existing text parser as
   a fallback when no snapshot exists (returns `status="none"`), so the demo
   never depends on a live Banner call.
2. **Add a `hokieday/classes.json` snapshot selection.** Prefer
   `config.DATA_DIR / "classes"` (live cache) then
   `config.FIXTURES_DIR / "classes_snapshot_*.json"`; expose the chosen
   `snapshot_id` + `is_stale` in the tool payload so the UI can show the
...
3. **Expose the search/list tools** (`find_classes`, `class_schedule`) through
   ...
   Return `search_result_json` / `schedule_json` verbatim.
...
- **Scraping fragility.** Banner is a legacy PL/SQL web app. Field names,
  ...
  changes.
- **Terms.** Only `202609` (Fall 2026) and `202612` (Winter 26-27) are listed;
  add a `TERM_WINDOWS` row (with a verified academic-calendar source) before
  expanding a new term.
...
- **No final-exam dates beyond the grid.** `parse_exam_schedule_html` reads the
  public grid but is conservative; it is a hook, not a planner input yet.
- **ICS subset.** Monthly/exception (`EXDATE`/`RDATE`), multi-VEVENT timezone
  overrides, and alarms are out of scope and flagged/ignored.
...
meeting merge, `search`/`select_crns`, add/remove/resolve schedule, recurrence
+ holiday skipping, conflict detection, next-class deadline, timezone (TZID /
UTC / floating), ICS happy + malformed + unsupported RRULE, building crosswalk
(no coordinates + GIS join validation), snapshot staleness + `sanitize_query`,
the edge script's required form fields and offline snapshot write, and privacy
exclusions (no network imports in the library, no PII keys in payloads).