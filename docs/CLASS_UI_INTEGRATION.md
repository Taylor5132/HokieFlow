...
Schedule → Find classes or import calendar connects the existing `hokieday/classes.py` contracts to the account calendar. The Events tab connects `hokieday/events.py` to the same calendar.
...
- UI compares imports against saved classes/events, including weekly entries. A search result has an **Add to schedule +** action that adds that section in one tap (timetable rows only; it implies the weekly-pattern consent, which the confirmation states along with the meeting count), and the ICS path keeps a review step that shows exact dated meetings and overlap warnings before a deliberate save. `classAddPlan` in `ui/class-import.js` owns the one-tap decisions (preview errors refuse the add, a saved section is skipped, the 200-entry cap holds, conflicts are reported separately from provenance notes) and is unit-tested in `tests/class-import.test.js`. The final write uses the existing authenticated, versioned PUT `/api/account/data`; all other account data is preserved.
