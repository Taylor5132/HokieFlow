...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, route) -> None:
        """Run a route, and turn any escaping exception into a real response.

        An unhandled exception in a handler thread writes nothing, so the client
        gets an empty reply -- which a proxy reports as 502 Bad Gateway with no
        clue why. That is exactly how the deployed site behaved when
        APP_ENV=production was set without SESSION_SECRET: auth.py correctly
        refuses to sign sessions with a development secret, and the request died
    ...
    def _failed(self, exc: BaseException) -> None:
        ...
        if self._responded:
            # Mid-response: the only honest move left is to end the connection.
            self.close_connection = True
            return
        configuration = isinstance(exc, RuntimeError)
        self._json({
            "error": ("accounts are unavailable: the server is missing required "
                      "configuration" if configuration else "internal error"),
            "type": type(exc).__name__,
            "status": "unavailable",
        }, 503 if configuration else 500)
