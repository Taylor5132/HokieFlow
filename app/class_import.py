...
def search_endpoint(query):
    """Catalog search for the Schedule tab.

    Local captures first (committed fixtures plus any runtime snapshots), with
    a live fill-in when they have nothing for this query: without it the UI
    answered "no timetable snapshot" for every subject nobody had captured,
    which is exactly how class search looked broken in production, where the
    runtime snapshot directory is empty.
    """
    term = str(query.get('term', '202609'))
    subject = str(query.get('subject') or '')
    course_number = str(query.get('course_number') or '')
    crn = str(query.get('crn') or '')
    snapshots = [s for s in sources() if s.term == term]
    live = class_search.live_snapshot(term, {'subject': subject,
                                             'course_number': course_number,
                                             'crn': crn})
    if live is not None:
        # Freshest first, so a live CRN wins over an older capture of it.
        snapshots = [live, *[s for s in snapshots if s.snapshot_id != live.snapshot_id]]
    if not snapshots:
    ...
    return {'state': 'results' if sections else 'no_results', 'sections': sections[:100],
            ...
            'source': 'banner_live' if live is not None else 'snapshot',
